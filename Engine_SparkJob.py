...
from MLPipeline.NamedEntityInference import NamedEntitiesInference
from MLPipeline.RequestParams import RequestParams
from MLPipeline.Config import Config

class SparkJob(Config):
    ...
    def check_complain_type(self, text):
        ...
        return str(multi.predict_complaint(text))

    def get_name_entities(self, text):
        name_entity = NamedEntitiesInference()
...
# ./streamVenv/lib/site-packages/pyspark/bin/spark-submit --packages org.apache.spark:spark-sql-kafka-0-10_2.12:3.1.2  ./Engine_SparkJob.py localhost 9999 --master local[*]
...
# kafka-console-consumer.sh --topic tweet-data --bootstrap-server localhost:29092

# docker run --net=host confluentinc/cp-kafka:4.0.0 kafka-console-consumer --bootstrap-server localhost:29092
